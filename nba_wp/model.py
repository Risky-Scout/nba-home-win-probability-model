...
def fit_logit_stacker(
    ...
    *,
    ...
    total = a + b
    if min_temperature is None or total <= 0.0:
        return stacker

    temperature = 1.0 / total
    if temperature + 1e-12 >= float(min_temperature):
        return stacker

    # Project onto T = min_temperature: a'+b' = 1/T, same weight w = a/(a+b).
    weight = a / total
    t_floor = float(min_temperature)
    a_proj = weight / t_floor
    b_proj = (1.0 - weight) / t_floor
    ...
    stacker.temperature_floor_ = t_floor
    return stacker
...
def stacker_calibration_dict(stacker: LogisticRegression) -> dict[str, Any]:
    ...
    payload: dict[str, Any] = {
        ...
        "temperature_floor": float(getattr(stacker, "temperature_floor_", 1.0)),
