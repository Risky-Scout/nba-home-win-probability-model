...
| Calibration-risk audit keeps the raw-Elo champion (identity-shrunk Platt/Beta over-correct out-of-sample under a strict multi-gate rule) → `keep_raw_elo` | `scripts/calibration_challenger.py`; `nba_wp/model.py` | `artifacts/calibration_challenger_decision.json`; `artifacts/calibration_challenger_frozen_predictions.csv` / `_daily_predictions.csv` | `tests/test_calibration_risk.py` |
